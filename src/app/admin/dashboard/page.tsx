...
import { Users, Store, TrendingUp, AlertCircle, LayoutDashboard, Settings, Package, CreditCard, LogOut, ChevronRight, Activity, ShieldCheck, Database } from "lucide-react";
...
export default function AdminDashboard() {
  const stats = [
    ...
    { title: "Faturamento SaaS", value: "R$ 125.400", icon: CreditCard, color: "text-green-600", bg: "bg-green-100", trend: "+8.4%" },
    { title: "Usuários Ativos", value: "12.8k", icon: Users, color: "text-purple-600", bg: "bg-purple-100", trend: "+5.1%" },
    { title: "Uptime Sistema", value: "99.98%", icon: Activity, color: "text-orange-600", bg: "bg-orange-100", trend: "Estável" },
  ];
    ...
    <div className="flex min-h-screen bg-slate-50">
      <aside className="w-64 border-r bg-white hidden lg:flex flex-col">
        ...
        <nav className="flex-1 px-4 space-y-2">
          ...
          <Link href="#" className="flex items-center gap-3 px-4 py-2.5 text-slate-600 hover:bg-slate-100 rounded-xl transition-colors font-medium">
            <Package className="h-5 w-5" /> Planos & Assinaturas
          </Link>
          <Link href="#" className="flex items-center gap-3 px-4 py-2.5 text-slate-600 hover:bg-slate-100 rounded-xl transition-colors font-medium">
            <Database className="h-5 w-5" /> Logs de Auditoria
          </Link>
          <Link href="#" className="flex items-center gap-3 px-4 py-2.5 text-slate-600 hover:bg-slate-100 rounded-xl transition-colors font-medium">
            <Settings className="h-5 w-5" /> Infraestrutura
          </Link>
        </nav>
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">Métricas da Plataforma</h1>
            <p className="text-slate-500">Controle total de multi-tenancy e performance.</p>
          </div>
          <div className="flex items-center gap-3">
             <Badge variant="secondary" className="bg-slate-200 text-slate-700 px-4 py-1 rounded-full font-medium">v2.8.1-stable</Badge>
          </div>
        </header>
        ...
        <div className="grid gap-6 lg:grid-cols-3 mb-8">
          <Card className="lg:col-span-2 border-none shadow-sm p-6 rounded-3xl">
             <CardHeader className="p-0 mb-6 flex flex-row items-center justify-between">
                <div>
                  <CardTitle className="text-xl">Crescimento de Receita (MRR)</CardTitle>
                </div>
             </CardHeader>
          ...
          <Card className="border-none shadow-sm p-6 flex flex-col justify-between rounded-3xl">
            <CardHeader className="p-0">
               <CardTitle className="text-xl">Saúde do Sistema</CardTitle>
               <p className="text-sm text-slate-500">Alertas de infraestrutura.</p>
            </CardHeader>
            <div className="space-y-4 mt-6">
               <div className="flex items-center gap-4 p-4 rounded-2xl bg-orange-50 border border-orange-100">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-orange-700">Aprovação de novos lojistas.</p>
                  </div>
               </div>
               <div className="pt-4 border-t">
                  <p className="text-xs font-bold text-slate-400 mb-4">Ações de Segurança</p>
                  <Button className="w-full bg-slate-900 rounded-xl mb-2">Ver Auditoria</Button>
                  <Button variant="outline" className="w-full rounded-xl">Manutenção Programada</Button>
               </div>
            </div>
          </Card>
        </div>
        ...
        <Card className="border-none shadow-sm rounded-3xl overflow-hidden">
          <CardHeader className="border-b bg-white flex flex-row items-center justify-between p-6">
            <CardTitle className="text-xl">Assinaturas Recentes</CardTitle>
          </CardHeader>
          <CardContent className="p-0">
            <Table>
              <TableHeader className="bg-slate-50">
                <TableRow>
                  <TableHead className="px-6 py-4">Lojista</TableHead>
                  <TableHead>Plano</TableHead>
                  <TableHead>Status</TableHead>
                  <TableHead className="text-right px-6">Assinado em</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {MOCK_MERCHANTS.map((merchant) => (
                  <TableRow key={merchant.id} className="hover:bg-slate-50">
                    <TableCell className="px-6 py-4 font-semibold text-slate-900">
                      {merchant.name}
                    </TableCell>
                    <TableCell>
                      <Badge variant="secondary" className="rounded-lg">{merchant.plan}</Badge>
                    </TableCell>
                    <TableCell>
                      <Badge className="bg-green-100 text-green-700 border-none">Ativo</Badge>
                    </TableCell>
                    <TableCell className="text-right px-6 text-slate-500 text-sm">
                      {merchant.createdAt}
                    </TableCell>
                  </TableRow>
                ))}
