...
export default function OnboardingPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 py-20 font-body">
      <div className="w-full max-w-3xl space-y-8">
        <div className="text-center space-y-2">
          <div className="inline-flex items-center justify-center h-16 w-16 bg-primary rounded-2xl text-white shadow-xl shadow-primary/20 mb-4 animate-pulse">
            <Sparkles className="h-8 w-8" />
          </div>
          <h1 className="text-4xl font-black text-slate-900 italic tracking-tighter uppercase leading-none">Configuração Enterprise</h1>
          <p className="text-slate-500 font-medium italic">Passo {step} de 5 • Dados para Conformidade</p>
        </div>
            ...
            {step === 1 && (
              <div className="space-y-6 animate-in fade-in">
                <div className="flex items-center gap-3 mb-6">
                   ...
                   <h3 className="text-xl font-black italic uppercase">Identidade do Negócio</h3>
                </div>
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase text-slate-400">Nome da Unidade</Label>
                    <Input required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="h-14 rounded-2xl bg-slate-50 border-none font-bold" />
                  </div>
                  <div className="space-y-2">
                ...
                <div className="space-y-2">
                  <Label className="text-[10px] font-black uppercase text-slate-400">Subdomínio Desejado (.agil.com)</Label>
                  <Input required value={formData.slug} onChange={e => setFormData({...formData, slug: e.target.value})} className="h-14 rounded-2xl bg-slate-50 border-none font-bold text-primary" />
                </div>
                <Button onClick={handleNextStep} className="w-full h-16 bg-slate-900 rounded-[30px] font-black italic shadow-xl">CONTINUAR</Button>
              </div>
            )}
            ...
            {step === 2 && (
              <div className="space-y-6 animate-in fade-in">
                <div className="flex items-center gap-3 mb-6">
                   ...
                   <h3 className="text-xl font-black italic uppercase">Dados Legais & CNPJ</h3>
                </div>
                <div className="space-y-2">
                  <Label className="text-[10px] font-black uppercase text-slate-400">CNPJ (Obrigatório)</Label>
                  <Input required placeholder="00.000.000/0000-00" value={formData.cnpj} onChange={e => setFormData({...formData, cnpj: e.target.value})} className="h-14 rounded-2xl bg-slate-50 border-none font-bold" />
                </div>
                <div className="space-y-2">
                  <Label className="text-[10px] font-black uppercase text-slate-400">Razão Social</Label>
                  <Input placeholder="Empresa LTDA" value={formData.razaoSocial} onChange={e => setFormData({...formData, razaoSocial: e.target.value})} className="h-14 rounded-2xl bg-slate-50 border-none font-bold" />
                </div>
                <div className="space-y-2">
                  ...
                  <Select value={formData.regimeTributario} onValueChange={v => setFormData({...formData, regimeTributario: v})}>
                    <SelectTrigger className="h-14 rounded-2xl bg-slate-50 border-none font-bold"><SelectValue /></SelectTrigger>
                    <SelectContent className="rounded-xl border-none"><SelectItem value="MEI">MEI</SelectItem><SelectItem value="Simples">Simples Nacional</SelectItem><SelectItem value="Presumido">Lucro Presumido</SelectItem></SelectContent>
                  </Select>
                </div>
                <div className="flex gap-4">
                  <Button variant="ghost" onClick={handlePrevStep} className="h-16 flex-1 rounded-[30px] font-black">VOLTAR</Button>
                  <Button onClick={handleNextStep} className="h-16 flex-[2] bg-slate-900 rounded-[30px] font-black italic">PRÓXIMO</Button>
                </div>
              </div>
            )}
            ...
            {step === 3 && (
              <div className="space-y-6 animate-in fade-in">
                <div className="flex items-center gap-3 mb-6">
                   ...
                   <h3 className="text-xl font-black italic uppercase">Localização & Contato</h3>
                </div>
                <div className="space-y-2">
                  <Label className="text-[10px] font-black uppercase text-slate-400">Endereço Completo</Label>
                  <Input value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} className="h-14 rounded-2xl bg-slate-50 border-none font-bold" />
                </div>
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase text-slate-400">WhatsApp Oficial</Label>
                    <Input value={formData.whatsapp} onChange={e => setFormData({...formData, whatsapp: e.target.value})} className="h-14 rounded-2xl bg-slate-50 border-none font-bold" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase text-slate-400">E-mail Administrativo</Label>
                    <Input value={formData.emailContatos} onChange={e => setFormData({...formData, emailContatos: e.target.value})} className="h-14 rounded-2xl bg-slate-50 border-none font-bold" />
                  </div>
                </div>
                <div className="flex gap-4">
                  <Button variant="ghost" onClick={handlePrevStep} className="h-16 flex-1 rounded-[30px] font-black">VOLTAR</Button>
            ...
            {step === 4 && (
              <div className="space-y-6 animate-in fade-in">
                <div className="flex items-center gap-3 mb-6">
                   ...
                   <h3 className="text-xl font-black italic uppercase">Operação & Finanças</h3>
                </div>
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase text-slate-400">Qtd. de Cadeiras/Boxes</Label>
                    <Input type="number" value={formData.chairs} onChange={e => setFormData({...formData, chairs: Number(e.target.value)})} className="h-14 rounded-2xl bg-slate-50 border-none font-bold" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase text-slate-400">Taxa Máquina Crédito (%)</Label>
                    <Input type="number" step="0.01" value={formData.creditFee} onChange={e => setFormData({...formData, creditFee: Number(e.target.value)})} className="h-14 rounded-2xl bg-slate-50 border-none font-bold" />
                  </div>
                </div>
                <div className="space-y-2">
                  <Label className="text-[10px] font-black uppercase text-slate-400">Chave PIX Oficial</Label>
                  <Input placeholder="CNPJ ou E-mail" value={formData.pixKey} onChange={e => setFormData({...formData, pixKey: e.target.value})} className="h-14 rounded-2xl bg-slate-50 border-none font-bold text-green-600" />
                </div>
                <div className="flex gap-4">
                  <Button variant="ghost" onClick={handlePrevStep} className="h-16 flex-1 rounded-[30px] font-black">VOLTAR</Button>
                  <Button onClick={handleNextStep} className="h-16 flex-[2] bg-slate-900 rounded-[30px] font-black italic">ESCOLHER PLANO</Button>
                </div>
              </div>
            )}
            ...
            {step === 5 && (
              <form onSubmit={handleCreateBusiness} className="space-y-8 animate-in fade-in">
                <div className="flex items-center gap-3 mb-6">
                   ...
                   <h3 className="text-xl font-black italic uppercase">Licenciamento Mensal</h3>
                </div>
                   ...
                   {SYSTEM_PLANS.map(plan => (
                     ...
                     >
                        <div className="flex items-center gap-4">
                           <div className={cn("h-12 w-12 rounded-2xl flex items-center justify-center", formData.planId === plan.id ? "bg-primary text-white" : "bg-white text-slate-400")}>
                           ...
                           <div>
                              ...
                              <p className="text-[9px] font-bold text-slate-400 uppercase">Até {plan.maxStaff} Profissionais</p>
                           </div>
                        </div>
