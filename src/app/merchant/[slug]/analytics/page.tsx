    ...
    <div className="flex min-h-screen bg-slate-50 dark:bg-slate-950 font-body">
      <aside className="w-64 border-r dark:border-slate-800 bg-white dark:bg-slate-900 hidden lg:flex flex-col sticky top-0 h-screen">
        ...
        <nav className="flex-1 px-4 space-y-2">
          ...
          <Link href={`/merchant/${slug}/settings`} className="flex items-center gap-3 px-4 py-2.5 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors font-medium">
      ...
      <main className="flex-1 p-4 lg:p-8 overflow-y-auto">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10">
          <div>
            <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tighter italic uppercase">Deep Analytics & BI</h1>
            <p className="text-slate-500 font-medium">Inteligência competitiva e comportamento do consumidor.</p>
          </div>
        ...
        <div className="grid gap-6 md:grid-cols-4 mb-10">
           {[
             ...
             <Card key={i} className="border-none shadow-sm rounded-[32px] overflow-hidden dark:bg-slate-900">
               <CardContent className="p-6">
                 <div className="flex justify-between items-start">
                    <div className={`${stat.bg} dark:bg-slate-800 p-3 rounded-2xl`}>
                       <stat.icon className={`h-6 w-6 ${stat.color}`} />
                    </div>
                    <Badge className="bg-slate-50 dark:bg-slate-800 text-slate-400 border-none font-black italic">{stat.trend}</Badge>
                 </div>
                 <div className="mt-4">
        ...
        <div className="grid gap-8 lg:grid-cols-3 mb-8">
           <Card className="lg:col-span-2 border-none shadow-sm rounded-[40px] p-8 bg-white dark:bg-slate-900">
              <div className="flex justify-between items-center mb-8">
                 <CardTitle className="text-2xl font-black italic dark:text-white">Mapa de Calor por Região</CardTitle>
                 <Map className="h-6 w-6 text-slate-200 dark:text-slate-700" />
              </div>
              <div className="h-[350px]">
                 <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={heatMapData}>
                       <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" opacity={0.1} />
                       <XAxis dataKey="region" axisLine={false} tickLine={false} tick={{fontWeight: 700, fill: '#94a3b8', fontSize: 10}} />
                       <YAxis axisLine={false} tickLine={false} tickFormatter={(v) => `R$ ${v/1000}k`} tick={{fontWeight: 700, fill: '#94a3b8', fontSize: 10}} />
                       <Tooltip cursor={{fill: '#f8fafc', opacity: 0.1}} contentStyle={{borderRadius: '20px', border: 'none', backgroundColor: '#1e293b', color: '#fff'}} />
                       <Bar dataKey="rev" radius={[15, 15, 0, 0]}>
                          {heatMapData.map((entry, index) => (
                            <Cell key={`cell-${index}`} fill={index === 0 ? 'hsl(var(--primary))' : '#334155'} />
                          ))}
                       </Bar>
                    </BarChart>
                 </ResponsiveContainer>
           ...
           <div className="space-y-6">
              <Card className="border-none shadow-sm rounded-[40px] p-8 bg-slate-900 dark:bg-black text-white relative overflow-hidden">
                 <div className="relative z-10">
                    ...
                    <p className="text-sm text-slate-400 mt-4 leading-relaxed font-medium italic">
              ...
              <Card className="border-none shadow-sm rounded-[40px] p-8 bg-white dark:bg-slate-900">
                 <CardTitle className="text-xl font-black italic mb-6 dark:text-white">Funil de Vendas</CardTitle>
                 <div className="space-y-4">
                    {conversionData.map((item, i) => (
                      <div key={item.name} className="space-y-1">
                         <div className="flex justify-between text-[10px] font-black uppercase tracking-widest">
                            <span className="text-slate-400">{item.name}</span>
                            <span className="text-slate-900 dark:text-slate-300">{item.value}</span>
                         </div>
        ...
        <Card className="border-none shadow-sm rounded-[40px] p-10 bg-white dark:bg-slate-900">
           <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
              <div className="space-y-1">
                 <h2 className="text-2xl font-black italic tracking-tighter uppercase dark:text-white">Previsão de Receita (IA Preditiva)</h2>
                 <p className="text-sm text-slate-400 font-bold">Baseado em tendências sazonais e comportamento de mercado.</p>
              </div>
              <Badge className="bg-primary/10 text-primary border-none px-4 py-2 font-black italic">CONFIANÇA IA: 92%</Badge>
           </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                 <AreaChart data={[
                   ...
                   { d: '30/10', v: 9500 }
                    ...
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" opacity={0.1} />
                    <XAxis dataKey="d" axisLine={false} tickLine={false} tick={{fontWeight: 700, fill: '#94a3b8', fontSize: 10}} />
                    <YAxis axisLine={false} tickLine={false} tick={{fontWeight: 700, fill: '#94a3b8', fontSize: 10}} />
